...
def questions(df,xuser):
         sections=['Lettres & langues OU Sc. Humaines','Médias','Sc. Juridiques','Arts et métiers','Médicale','Gestion et sc. Économiques','Sc. Fondamentales','Agriculture & environnement','Etudes ingénieurs OU Etudes technologiques']




         s={'Lettres & langues OU Sc. Humaines': """Faculté des Sciences Humaines et Sociales de Tunis \n
...
ESPRIT Prepa\n"""
         ...
         d={'Lettres & langues OU Sc. Humaines': """ Les Lettres et Sciences Humaines sont destinées à ceux qui s'intéressent à la littérature, aux langues, à l'histoire-géo… et qui sont curieux des différentes formes de culture littéraire fondée sur l'analyse, la mise en perspective d'une œuvre et l'argumentation. """
...
- Dispenser les notions fondamentales, les fondements théoriques et méthodologiques en Droit et en Sciences Politiques permettant aux étudiants de comprendre, de maîtriser les dimensions pratiques du droit et d’analyser le fonctionnement des institutions Nationales et Internationales.""" 
         ...
         ypred = LR(X,y,xuser_encoded.values.reshape(1, -1))
         st.write("\n")
         st.write("\n")
         st.write('\n')
         st.header("*On recommande :*")
         st.subheader(sections[ypred[0]-1]+" :")
         st.write('\n')
         st.write(d[sections[ypred[0]-1]])
         st.write('\n')
         st.header("*Vous pouvez choisir parmis ces écoles :*")
         st.write('\n')
         st.write(s[sections[ypred[0]-1]])
...
def preprocessing(df):
	df['Quel est ton parcours académique ?'] = np.where(df['Quel est ton parcours académique ?'] == 'Lettres & langues',1,df['Quel est ton parcours académique ?'])
